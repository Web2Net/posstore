...
if($event!=""){
$tag_data=CartTag::tagData($id);

if($tag_data["level"]==1){
CartTag::Event1($event,CART_TAG_TABLE,$id);
}
CartTag::Event2($event,CART_TAG_TABLE,$id);
}
	
$tag1_list=CartTag::tagListIdData($level=1);
$tag2_list=CartTag::tagListIdData($level=2);
...
if($arr_value['level']==2){
	$tag_connect_list=CartTag::tagListConnectData($id);
    ...
    $c_tag_data=CartTag::tagData($val);
...
if($arr_value['level']==2){	
	...
	rename(CART_ITEM_IMG_PATH."/new",CART_ITEM_IMG_PATH."/".$id);
...
/*
if($event!=""){FotoCart::Event($event,'items_branch',$id);}
if($com=="add"){
